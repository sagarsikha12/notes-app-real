...
class ConnectionPool extends mongo_types_1.TypedEventEmitter {
    ...
    checkOut(callback) {
        ...
        if (waitQueueTimeoutMS) {
            waitQueueMember.timer = setTimeout(() => {
                ...
                waitQueueMember.callback(new errors_1.WaitQueueTimeoutError(this.loadBalanced ? this.waitQueueErrorMetrics() : '', this.address));
            }, waitQueueTimeoutMS);
        }
    ...
    clear(serviceId) {
        if (this.loadBalanced && serviceId) {
            ...
            if (generation == null) {
                throw new error_1.MongoDriverError('Service generations are required in load balancer mode.');
            }
            else {
     ...
     * @param callback - The original callback
     */
    withPinnedConnection(session, fn, callback) {
        const pinnedConnection = session.pinnedConnection;
        if (!pinnedConnection) {
            this.checkOut((err, conn) => {
                if (conn) {
                    executePinned(err, conn, fn, callback);
...
function executePinned(error, connection, fn, callback) {
