...
let Cursor = class Cursor extends abstract_cursor_1.AbstractCursor {
    ...
    async hasNext() {
        if (this._tailable) {
            deprecation_warning_1.printWarning('If this is a tailable cursor with awaitData, and there are no documents in the batch, this method ' +
                'will will block. Use tryNext if you want to check if there are any documents without waiting.', this._mongo._internalState.context.print);
        }
    ...
    async next() {
        if (this._tailable) {
            deprecation_warning_1.printWarning('If this is a tailable cursor with awaitData, and there are no documents in the batch, this' +
                ' method will will block. Use tryNext if you want to check if there are any documents without waiting.', this._mongo._internalState.context.print);
        }
        return super.next();
    }
    noCursorTimeout() {
