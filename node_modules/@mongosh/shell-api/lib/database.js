...
let Database = class Database extends decorators_1.ShellApiWithMongoClass {
    ...
    async _runCommand(cmd, options = {}) {
        return this._mongo._serviceProvider.runCommandWithCheck(this._name, cmd, { ...this._mongo._getExplicitlyRequestedReadPref(), ...this._baseOptions, ...options });
    }
    async _runAdminCommand(cmd, options = {}) {
        return this._mongo._serviceProvider.runCommandWithCheck(enums_1.ADMIN_DB, cmd, { ...this._mongo._getExplicitlyRequestedReadPref(), ...this._baseOptions, ...options });
    }
    async _listCollections(filter, options) {
        return await this._mongo._serviceProvider.listCollections(this._name, filter, { ...this._mongo._getExplicitlyRequestedReadPref(), ...this._baseOptions, ...options }) || [];
    }
    ...
    async _getLastErrorObj(w, wTimeout, j) {
        ...
        try {
            return await this._mongo._serviceProvider.runCommand(this._name, cmd, this._baseOptions);
        }
        catch (e) {
            return e;
        }
    ...
    async aggregate(pipeline, options) {
        ...
        const providerCursor = this._mongo._serviceProvider.aggregateDb(this._name, pipeline, { ...this._baseOptions, ...aggOptions }, dbOptions);
    ...
    async dropDatabase(writeConcern) {
        return await this._mongo._serviceProvider.dropDatabase(this._name, { ...this._baseOptions, writeConcern });
    }
    ...
    async createCollection(name, options = {}) {
        ...
        return await this._mongo._serviceProvider.createCollection(this._name, name, { ...this._baseOptions, ...options });
    }
    ...
    async enableFreeMonitoring() {
        ...
        await this._mongo._serviceProvider.runCommand(enums_1.ADMIN_DB, {
            setFreeMonitoring: 1,
            action: 'enable'
        }, this._baseOptions);
        let result;
        let error;
        try {
            result = await this._mongo._serviceProvider.runCommand(enums_1.ADMIN_DB, { getFreeMonitoringStatus: 1 }, this._baseOptions);
        }
        catch (err) {
            error = err;
        }
        ...
        if (result.state !== 'enabled') {
            const urlResult = await this._mongo._serviceProvider.runCommand(enums_1.ADMIN_DB, {
                getParameter: 1,
    ...
    async getLastErrorObj(w, wTimeout, j) {
        deprecation_warning_1.printDeprecationWarning('Database.getLastErrorObj() is deprecated and will be removed in the future.', this._mongo._internalState.context.print);
    ...
    async getLastError(w, wTimeout) {
        deprecation_warning_1.printDeprecationWarning('Database.getLastError() is deprecated and will be removed in the future.', this._mongo._internalState.context.print);
    ...
    async printShardingStatus(verbose = false) {
        ...
        const result = await helpers_1.getPrintableShardStatus(this, verbose);
        return new index_1.CommandResult('StatsResult', result);
    }
    async printSecondaryReplicationInfo() {
        let startOptimeDate = null;
        const local = this.getSiblingDB('local');
        const admin = this.getSiblingDB(enums_1.ADMIN_DB);
