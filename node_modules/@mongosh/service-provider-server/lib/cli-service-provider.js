...
exports.connectMongoClient = void 0;
...
async function connectWithFailFast(client, bus) {
    ...
    const heartbeatFailureListener = ({ failure, connectionId }) => {
        ...
        const isFailFast = service_provider_core_1.isFastFailureConnectionError(failure);
...
async function resolveMongodbSrv(uri, bus) {
...
function detectAndLogMissingOptionalDependencies(bus) {
    try {
        require('saslprep');
    }
    catch (error) {
...
async function connectMongoClient(uri, clientOptions, bus, MClient = mongodb_1.MongoClient) {
    var _a, _b;
    ...
    if (clientOptions.autoEncryption !== undefined &&
        ...
        if (!((_a = buildInfo.modules) === null || _a === void 0 ? void 0 : _a.includes('enterprise')) &&
            ...
            throw new errors_1.MongoshRuntimeError('Automatic encryption is only available with Atlas and MongoDB Enterprise');
        }
    }
...
class CliServiceProvider extends service_provider_core_1.ServiceProviderCore {
    ...
    static async connect(uri, driverOptions = {}, cliOptions = {}, bus = new events_1.EventEmitter()) {
        ...
        const mongoClient = !cliOptions.nodb ?
            await connectMongoClient(connectionString.toString(), clientOptions, bus) :
    ...
    async getNewConnection(uri, options = {}) {
        ...
        const mongoClient = await connectMongoClient(connectionString.toString(), clientOptions, this.bus);
    ...
    async resetConnectionOptions(options) {
        ...
        const mc = await connectMongoClient(this.uri.toString(), clientOptions, this.bus);
        try {
