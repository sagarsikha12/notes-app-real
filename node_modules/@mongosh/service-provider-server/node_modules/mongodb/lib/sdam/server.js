...
const PINNED_NO_SESSION = new error_1.MongoDriverError('Cannot execute on pinned connection without a session.');
/** @internal */
...
class Server extends mongo_types_1.TypedEventEmitter {
    ...
    command(ns, cmd, options, callback) {
        ...
        if (this.loadBalanced && isPinnableCommand(cmd, session)) {
            if (session) {
                this.s.pool.withPinnedConnection(session, (err, conn, cb) => {
                    if (err || !conn) {
                        return cb(err);
                    }
...
function markServerUnknown(server, error) {
    ...
    if (!server.loadBalanced) {
        ...
        server.emit(Server.DESCRIPTION_RECEIVED, new server_description_1.ServerDescription(server.description.hostAddress, undefined, {
            error,
            topologyVersion: error && error.topologyVersion
    ...
    return function handleOperationResult(err, result) {
        if (err && !connectionIsStale(server.s.pool, connection)) {
            if (err instanceof error_1.MongoNetworkError) {
                ...
                if (error_1.isSDAMUnrecoverableError(err) && !server.loadBalanced) {
