            ...
            if (err) {
                ...
                if (options.loadBalanced) {
                    if (options.replicaSet) {
                        return callback(new error_1.MongoParseError(LB_REPLICA_SET_ERROR));
                    }
                    if (hostAddresses.length > 1) {
                        return callback(new error_1.MongoParseError(LB_SINGLE_HOST_ERROR));
                    }
                }
            }
...
function checkLoadBalancerOptions(options, addresses) {
    if (options.loadBalanced) {
        if (options.replicaSet) {
